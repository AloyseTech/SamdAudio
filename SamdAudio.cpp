...
#define N_CHANNELS          4      //could be 1,2 or 4
#define AUDIO_BUFFER_SIZE   512
#define RAMPIN              255

/*Global variables*/
File __audioFile[N_CHANNELS];
volatile bool __audioPlaying[N_CHANNELS]={false};
volatile bool __audioLooping[N_CHANNELS][2]={false};
volatile bool __audioFileReady[N_CHANNELS] = {false};
volatile uint32_t __SampleIndex[N_CHANNELS];
uint8_t __WavSamples[N_CHANNELS][2][AUDIO_BUFFER_SIZE];
uint16_t rampDivisor[N_CHANNELS]={1};
uint8_t whichBuffer[N_CHANNELS]={0};
volatile bool fillNextBuffer[N_CHANNELS]={true};
volatile uint16_t __audioData;

uint8_t __AudioPlayerVolume=4; //0 to 3.
...
/*
 ...
 for(uint8_t index=0; index<N_CHANNELS; index++)
 {
 
 }
 */

void SamdAudio::begin(uint32_t sampleRate) {
...
void SamdAudio::loopChannel(uint8_t c, bool loopEnable)
{
    __audioLooping[c][0]=loopEnable;
}

void SamdAudio::setGain(uint8_t v)
{
    __AudioPlayerVolume=constrain(v,0,4);
}

void SamdAudio::gainUp()
{
    if(__AudioPlayerVolume<4)
        __AudioPlayerVolume++;
}

void SamdAudio::gainDown()
{
...
bool SamdAudio::isPlaying(uint8_t c)
{
    return __audioPlaying[c%N_CHANNELS];
}
        ...
        for(uint8_t index=0; index<N_CHANNELS; index++)
        {
            if (__audioPlaying[index])
            {
                ...
                else if (__audioFileReady[index])
                {
                    ...
                    {
                        /*__SampleIndex[index] = 0;
                        __audioLooping[index][1]=true;//__audioFile[index].seek(44); //looping : go back to beginning
                        fillNextBuffer[index]=1;
                        rampDivisor[index]=__WavSamples[index][1-whichBuffer[index]][AUDIO_BUFFER_SIZE - 1];*/
